...
def feature_standardise(features_df:pd.DataFrame) -> pd.DataFrame:
    scaler = StandardScaler()
    features_scaled = scaler.fit_transform(features_df[features].values)

    return pd.DataFrame(features_scaled, columns=features, index=features_df.index)